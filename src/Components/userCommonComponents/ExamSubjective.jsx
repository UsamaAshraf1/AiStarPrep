...
import { extractExamTextFunApi } from "../../store/exam/examServices";
...
const ExamSubjective = ({
  ...
  const handleImageUpload = async (event) => {
    ...
    event.target.value = null; // Clear input field to allow re-upload
    setImages([]);
    setLoading(true);

    try {
      const resultAction = await dispatch(extractExamTextFunApi(files));
      if (extractExamTextFunApi.fulfilled.match(resultAction)) {
  ...
  return (
    <div>
      ...
      <div className="w-full flex flex-wrap justify-center">
        {/* <p className="w-full text-[18px] font-medium text-[#1A1A1A] text-center leading-relaxed">
          {question?.question}
        </p> */}
      ...
      <div className="relative flex justify-center mt-6">
        ...
        <label className="absolute bottom-1 right-2 md:right-16 bg-[#0072CF] text-white p-2 rounded-full cursor-pointer shadow-md">
          <LuUpload size={20} />
          <input
            type="file"
            accept="image/*"
